...
int startTime(MPI_Comm comm, double *tic_p)
{
  ...
  int err;
  err = MPI_Barrier(comm); MPI_CHK(err);
  *tic_p = MPI_Wtime();
  //MPI_Barrier(comm);
  return 0;
}
...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (i == rank) {
      ...
      if (i) {
        err = MPI_Ssend(procname, namelen, MPI_CHAR, 0, 0, comm); MPI_CHK(err);
      }
    }
    if (!rank) {
      if (i) {
  ...
  for (int numComm = 4; numComm <= size; numComm *= 4) {
    for (int numBytes = 8; numBytes <= maxSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
          if (rank % 2) {
            err = MPI_Ssend(buffer, numBytes, MPI_BYTE, rank - 1, 0, comm); MPI_CHK(err);
          } else {
    ...
    for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           */
          //int count = numBytes * size;
          err = MPI_Scatter(buffer, numBytes, MPI_BYTE, buffer2, numBytes, MPI_BYTE, 0, subComm); MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
          timeAvg += tic;
    ...
    for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           * `buffer` on process 0.
           */
          err = MPI_Gather(buffer2, numBytes, MPI_BYTE, buffer, numBytes, MPI_BYTE, 0, subComm); MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
          timeAvg += tic;
    ...
    for (int numBytes = 8; numBytes <= maxSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           */
          err = MPI_Allreduce(MPI_IN_PLACE, buffer, numBytes, MPI_CHAR, MPI_MIN, subComm); MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
          timeAvg += tic;
    ...
    for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           * `buffer`.
           */
          err = MPI_Allgather(buffer2, numBytes, MPI_BYTE, buffer, numBytes, MPI_BYTE, subComm); MPI_CHK(err);
        }
        err = stopTime(tic, &tic); MPI_CHK(err);
        if (t) {
          timeAvg += tic;
    ...
    for (int numBytes = 8; numBytes <= maxCollectiveSize; numBytes *= 8) {
      ...
      for (int t = 0; t < numTests; t++) {
        ...
        if (rank < numComm) {
           ...
           */
          err = MPI_Alltoall(MPI_IN_PLACE, numBytes, MPI_BYTE, buffer, numBytes, MPI_BYTE, subComm); MPI_CHK(err);
